 ...
 * 其实做分组TopN
 */
object PaymentModel2 extends BaseModel{
 ...
 |-- counts: long (nullable = false)
    ...
    val sql :String =
      """
        ...
        |row_number() over(partition by userId,paymentCode order by counts desc) rn
        |from t_temp
        |""".stripMargin
    val rankDF2: DataFrame = spark.sql(sql).where('rn===1)
    rankDF2.show(10,false)
    /*
 +---------+-----------+------+---+
